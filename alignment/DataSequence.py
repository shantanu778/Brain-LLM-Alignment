...
class DataSequence(object):
    ...
    """
    def __init__(self, data, split_inds, vocab_file=None, flag=None, data_times=None, tr_times=None):
        ...
        if flag == "char":
            ...
            # self.str_to_ind =  {ch:i for i, ch in enumerate(self.vocab)}
            # self.ind_to_str =  {i:ch for i, ch in enumerate(self.vocab)}
            print(f"Loading meta from {vocab_file}...")
    ...
    def decode(self, text):
        return ''.join([self.itos[s] for s in text])
    
    def word_tokenize(self, text, tokenizer):
        tokens = tokenizer(text)
        indices = [self.vocab[t] for t in tokens]
        return indices
        
    def gpt_tokenize(self, text, tokenizer):
    ...
    @classmethod
    def from_grid(cls, vocab_file, flag, grid_transcript, trfile):
        ...
        return cls(data, split_inds, vocab_file, flag, word_avgtimes, trtimes+tr/2.0)
